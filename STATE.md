...
| Área | Status |
|------|--------|
| **v14 IdeiaOS Cockpit** | 🟢 v14.0 PRONTO P/ EXECUTAR (2026-06-21) — planejado (7 PLAN.md, 20 tasks/3 ondas) + **revisão de prontidão pré-exec** (GO_WITH_FIXES) → **5/7 planos corrigidos** (mktree 2-níveis, gate credencial discriminante, `node:sqlite`, ACs satisfazíveis) + verify adversarial **13/13 sondas** → CLEAR_TO_EXECUTE, commitado `ac68817`. v13 já tagou → gate de execução liberado. **Próximo: `/gsd-execute-phase 14.0` em contexto fresco** (⚠️ autosync PAUSADO — religar ao fim). Blueprint+contrato `/spec` vivo (`specs/cockpit/spec.md`, 9 req) + ADR + apuração 100% (20 docs). Aberto 🔴: origin-auth v14.4 (gate, não milestone) |
