...
- ✅ **nfideia spec PR — RESOLVIDO (2026-06-19):** specs do piloto portadas p/ `main` via **PR limpo [nfideia#40](https://github.com/Ideia-Business/nfideia/pull/40)** (`spec/pilot-port`, cherry-pick doc-only sobre a main atual, 6 arquivos 100% em `specs/`, zero código). Branch stale `spec/multi-tenancy-pilot` (71 atrás) **não** foi arrastada. Achado: o `fix(nfse-retry-stuck)` da branch original **já está na main** (não-órfão); a metodologia `/spec` já é viva na main (`onda1-honorarios`). Autosync pausado/religado durante a cirurgia. **Resta:** você revisar/mergear o PR #40 (Lovable-safe; conferir se `PILOT-BACKLOG.md` ainda é atual).
