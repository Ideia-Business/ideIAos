...
| **`scripts/check-manifest-drift.sh`** | **Gate de drift manifesto↔disco (v16)** — detecta componente de `source/` ausente do `manifests/modules.json`. Dois regimes: hook/skill/agent são **1:1** (órfão = WARN acionável); rules são por-**cobertura** (arquivo OU grupo — reporta as não-catalogadas sem falhar, pois a granularidade é decisão pendente). `--strict` (exit 1 se órfão 1:1), `--quiet`. Consumido pelo `idea-doctor §18`. Fecha o gap "drift manifesto↔disco não detectado por nenhum gate". |
