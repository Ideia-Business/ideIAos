...
**ADR:** `docs/decisions/v13-security-freshness-gate.md` (**Proposto** — aguarda aprovação p/ construir).
**Disciplina:** gatilho determinístico · LLM só na revisão · proporcionalidade (risco × idade) · zero dep nova · SOAK antes de tag.
**Status:** 📋 **PLANEJADO** — zero código; este pacote é o contrato para revisão antes da implementação.
...
| R13-03 | Risk-weighting config em `core-config.yaml` — pesos (3/1/0), globs por superfície, limiares (N=10/20, T=90/180d, crítico-30d); defaults conservadores e **tunáveis** | à prova de gaming | 📋 |
...
| R13-08 | Testes — caminho de FALHA (ledger stale → tier egrégio → tag bloqueada) no padrão `test-lovable-mcp-containment.sh` + wiring no CI (`evals.yml`) | prova o gate | 📋 |
| R13-09 | Propagação aos 5 sistemas (IdeiaOS + 4 produtos) com **bootstrap-baseline** em cada + ADR/plan/README/STATE/handoff | rollout | 📋 |
