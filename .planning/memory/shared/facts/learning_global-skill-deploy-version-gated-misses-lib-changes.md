...
**Como aplicar:**
- Para corrigir o drift NA HORA: espelhe o diretório — `cp -Rp source/skills/<s>/. ~/.claude/skills/<s>/` e confirme com `diff -rq` (exit 0 = idêntico). Não confie no setup p/ isso.
- Ao adicionar/alterar arquivos em `lib/`/`references/`/`data/` de uma skill, **bump a versão no SKILL.md** OU espelhe o global manualmente, senão o global fica defasado em silêncio.
- Defeito latente do `setup.sh`: o copy de skill deveria ser **content-aware** (`diff -rq` antes de pular), não version-gated. Ver [[learning-design-suite-sha-pin-clone-destructive]] (outra dica de remediação do doctor que engana).
