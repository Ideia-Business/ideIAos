...
**Refinamento (auditoria de fechamento v10, 2026-06-18):** ao declarar a impossibilidade, distinga **instrumento ≠ interface** — "impossível pelo INSTRUMENTO escolhido (o fork sem gitsync)" é quase sempre o verdadeiro; "impossível pela INTERFACE inteira (o MCP)" é uma superafirmação que fecha caminhos válidos. A2 era inmensurável **no fork**, mas **é** mensurável via MCP num **produto real com gitsync** (push divergente + deploy_project + ler bundle). Prefira sempre a versão mais estreita que a evidência sustenta — o veredito BLOQUEAR estava certo; só o rótulo precisava da correção.

Evidência: milestone v10 Lovable MCP, Fase B — o MCP acopla Cloud↔GitHub (provado read-only) mas **não expõe/gerencia o gitsync** (sem connector github, `get_project` sem URL de repo, `add_connector` negado, fork sem repo via `gh search commits`) → A2/A1-lag inmensuráveis **no fork** → veredito BLOQUEAR. Repo: `docs/learnings/2026-06-18-mcp-wrapper-may-hide-the-layer-your-experiment-depends-on.md`. Fechamento: `.planning/v10-MILESTONE-AUDIT.md`.
