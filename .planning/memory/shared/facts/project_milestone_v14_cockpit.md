...
description: v14 "IdeiaOS Cockpit" — console web CTO local-first. v14.0 (Substrato+Espinha) EXECUTADO/COMPLETO 2026-06-21. v14.1 (MVP Bridge) EXECUTADO/code-complete + CLOSEOUT 2026-06-22 (bug CORS-preflight S-05 corrigido). v14.4 (write-path security) contrato /spec MERJADO 2026-06-22 (specs/cockpit/spec.md 9→20 reqs) + ADRs Q1-Q3 ACEITOS (docs/decisions/v14.4-*; O2 + lista pinada local) + bootstrap Wave1+B2 EXECUTADO (source/agentd/{sign,verify,pinned-keys}.sh, 19/19 proof-gates por exit-code; B0/B1/B2 — o CRÍTICO de revogação forjada PROVADO fechado; ssh-keygen -Y, cripto-local). R-WP10 segue FECHADO; próximo = B3 Touch ID (autonomous:false) + B4 gate agregado. Cerimônia N=2-real + Q5 abertas. Tags v14.0/v14.1 DEFERIDAS (1 máq/0d). GOTCHA: CLI GSD não resolve fases v14 (sem v14-ROADMAP.md) → planejar/fechar espelhando o método v14.0.
metadata:
  node_type: memory
  type: project
