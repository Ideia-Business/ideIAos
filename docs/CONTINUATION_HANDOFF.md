...
> **▶ RETOMAR AQUI (2026-06-22 tarde — v14.4 contrato `/spec` MERJADO + ADRs Q1-Q3 PROPOSTOS; próximo = ACEITAR os ADRs OU 2ª máquina p/ tag) — leia primeiro:**
> Sessão "faça isso tudo": entregue o único 🔴 que era design sem código — o **contrato `/spec` de segurança do write-path v14.4** — e o **ratifiquei** (operador autorizou) + escrevi os **ADRs de mecanismo** (Q1/Q2/Q3).
...
> - **ADRs Q1-Q3 PROPOSTOS** em `docs/decisions/`: `v14.4-origin-auth-signing-mechanism.md` (Q1 bootstrap + Q2 revogação) recomenda **O2** (par por-máquina no keychain) — **fato verificado: signing-git NÃO bootstrapado nesta máquina** → O3 descartado; `v14.4-step-up-without-relying-party.md` (Q3) = LocalAuthentication-via-agentd + fallback terminal. Verificação adversarial 2 lentes = SOUND-WITH-FIXES; **CRÍTICO** pego e corrigido (host comprometido assinaria a revogação da chave BOA via ref → DoS): fix = **lista pinada autoritativa-LOCAL** (ref = espelho não-confiável) + papel ligado à chave pinada + revogação só por re-pin out-of-band. **Commit `4bb25ba` pushado.**
> - **🚦 PRÓXIMO PASSO (decisão SUA):** **revisar + aceitar os 2 ADRs** (status PROPOSTO → Aceito) — ou redirecionar o mecanismo (O2 vs O3, etc.). O gate **R-WP10 segue FECHADO**: aceitar os ADRs é necessário mas NÃO suficiente — falta o **bootstrap verificado por exit-code** (gerar chaves, provar assinar→verificar fail-closed + rejeição de revogação-forjada) antes de qualquer código v14.4 cross-máquina. **Q5** (ref de comando ao `origin`) fica **aberta/fora-de-escopo** dos ADRs — precisa de decisão própria no mesmo gate.
