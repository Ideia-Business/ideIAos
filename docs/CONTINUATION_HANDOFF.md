...
- **Deno runtime** instalado (`~/.local/bin`, v2.9.0) + `scripts/install-deno.sh` idempotente — mata o aviso recorrente "Deno não instalado" (era fallback de `deno test` em edge functions). Endurecido por review adversarial (8 achados). Memória [[reference-deno-install-local-bin]]. ✅ **WIRING DURÁVEL p/ TODA a frota (fix desta sessão 2026-06-30):** o instalador estava **órfão** (existia, mas nada o auto-rodava — só o doctor §17 avisava) → **cabeado no `setup.sh` step 6.3 (fase global)**. Como o branch global da propagação (`propagate-if-changed.sh:226`) e o `sync-all.sh` ambos rodam `setup.sh --global-only`, esse **único ponto** cobre máquina nova + refresh + a frota inteira no próximo ciclo de sync — não só quem roda o setup do zero. Fail-soft (runtime opcional; falha de rede não aborta o setup). Provado por exit-code: `bash -n setup.sh`=0, `install-deno.sh --quiet`=0 (no-op idempotente), lógica do step 6.3=0. `install-global-patches.sh` NÃO tocado (evitou renumerar 15 patches; a propagação já passa pelo `--global-only` antes dele).
