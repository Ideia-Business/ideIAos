...
> **▶ RETOMAR AQUI (próxima sessão 2026-06-19+) — leia primeiro:** validação de resíduos um-a-um está em **4/5**. Itens 1–4 FECHADOS nesta sessão (verify-deploy `IN_SYNC` · ENV-04 · Mac mini baixo-risco · nfideia spec → PR [nfideia#40](https://github.com/Ideia-Business/nfideia/pull/40)). **PRÓXIMO = item 5: revisar os 2 stashes** (ainda NÃO inspecionados nesta sessão):
> - `nfideia stash@{0}` — handoff trivial regenerável → provável **drop** (confirmar conteúdo antes).
> - `ideiapartner stash@{1}` — **type-safety pass real, ~20 arquivos → NÃO dropar sem revisar**; decidir apply/keep/drop após ver o diff.
>
> Plano da retomada: `git -C ~/dev/<repo> stash show -p stash@{N}` para cada um, mostrar conteúdo, decidir junto. ⚠️ Se for aplicar/mexer (cirurgia git), **pausar o autosync antes** (`launchctl bootout gui/$(id -u)/com.ideiaos.gitautosync`) e religar depois (`bootstrap … ~/Library/LaunchAgents/com.ideiaos.gitautosync.plist`) — padrão usado com sucesso nesta sessão. **Resíduo user-only paralelo:** mergear [nfideia#40](https://github.com/Ideia-Business/nfideia/pull/40) (Lovable-safe).
