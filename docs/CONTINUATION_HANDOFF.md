...
> **▶ RETOMAR AQUI (2026-06-22 tarde — v14.4: contrato MERJADO + ADRs ACEITOS + bootstrap B0/B1/B2 EXECUTADO; próximo = B3 (Touch ID) + B4, OU 2ª máquina p/ tag) — leia primeiro:**
...
> - **🚦 PRÓXIMO PASSO:** **B3** (step-up — `LocalAuthentication`/Touch ID, `autonomous:false`, **exige você**) + **B4** (gate agregado `scripts/test-writepath-bootstrap.sh`: manifesto de casos + exit-code/REASON específico + canário + gate-negativo + nonce-durável-cross-processo). 5/5 verde = pré-condição R-WP10 satisfeita (**mono-máquina**). **Aberto registrado** (não over-read do verde): cerimônia **N=2 real** (2º host físico) + **Q5** (ref ao `origin`) → fase de feature.
