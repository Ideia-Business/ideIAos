...
**Próximo (2 trilhas gated/opt-in):**
1. **Rollout operacional (seu)** — aplicar o snippet de deny do `mcp-protocol.md` ao `.claude/settings.json` de cada produto Lovable; no painel, desligar `mcp_enabled` nos 2 workspaces não-dev ("Grupo IDeia - Projects" + "Dev's Lovable"). Só então rodar `/lovable-mcp verify-deploy` contra um produto real.
2. **Fase B (sandbox)** — quando houver apetite por write-path: `remix_project` mede namespace/timing do mirror GitHub↔Cloud (gate de TODA escrita). C/D dependem de B.
