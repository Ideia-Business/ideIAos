...
> - **🚦 PRÓXIMO PASSO (gated, owner):** (1) **hardening** — a Resend key setada é **Full-access** (usada p/ destravar/listar domínios); trocar por **Sending-only** restrita a `updates.ideiabusiness.com.br` (`supabase secrets set RESEND_API_KEY='re_SENDING_ONLY' --project-ref xdikjgpkiqzgebcjgqmu`) + revogar a Full-access. (2) **cerimônia N=2 real** (re-pin out-of-band num 2º host) + **Q5** (ref ao origin) → só então a feature cross-máquina destrava; **R-WP10 segue FECHADO** (mono-máquina não discharge). (3) Fine-grained PATs na org `Ideia-Business` (owner). Limpeza opcional dos OTPs de teste: `delete from public.otp_codes; delete from public.otp_attempts;` no projeto `xdikjgpkiqzgebcjgqmu`.
>
