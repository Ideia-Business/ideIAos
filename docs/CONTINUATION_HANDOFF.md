...
> - **🚦 PRÓXIMO PASSO:** **FALTA só `RESEND_API_KEY`** (entrega de e-mail de produção, NÃO cripto): `supabase secrets set RESEND_API_KEY='re_…' STEPUP_MAIL_FROM='…' --project-ref xdikjgpkiqzgebcjgqmu`. Depois: **cerimônia N=2 real** (re-pin out-of-band num 2º host) + decisão **Q5** (ref ao origin) → só então a feature cross-máquina destrava. **R-WP10 segue FECHADO** (mono-máquina não discharge). Fine-grained PATs na org `Ideia-Business` segue ação do owner. Limpeza opcional do backend de teste: `delete from public.otp_codes; delete from public.otp_attempts;`.
>
