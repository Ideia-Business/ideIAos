...
- **Higiene lateral (mesma sessão):** ao mapear credenciais, achei 3 JWTs literais em 2 docs de `.planning/14.0-05-*`. **Verificado = NÃO era credencial** — fixtures "veneno" do Zero-Leak (payload `{"role":"service_role"}` dummy, sem `ref`/`iss`/`exp`, sig `AAAA…`; zero em código executável; gates não escaneiam `.planning`). Mesmo benigno, **neutralizei os 3 literais** por placeholder (higiene de repo público) preservando a narrativa pedagógica. Repo agora com **0 JWT completo colável**.
...
- Sessão salva em: `/Users/gustavolopespaiva/.claude/sessions/2026-07-02-ideiaos-27e39c97-b8fe-446e-b3a2-6242e36d.tmp`
