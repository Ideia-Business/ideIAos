...
> - **Tag `v14.0` FECHADA** (SOAK re-record → span 1d, gate exit 0, `git tag v14.0` + push via @devops). `v14.1`: o span ≥1d só abre **≥2026-06-23T10:59:55** → re-record (`check-soak.sh v14.1 --record`) + commit/push + `git tag v14.1` depois desse horário.
...
> - **🚦 PRÓXIMO PASSO:** **F0b** (`autonomous:false`, exige você) = provisionar o projeto Supabase DEDICADO `ideiaos-cockpit-stepup` + aplicar `schema.sql` + setar `STEPUP_SIGNING_KEY`/`KID` + deployar as 4 edge functions + **enrollment out-of-band** da pubkey (`bash source/agentd/stepup-pin-backend.sh add <kid> <spki>`) + fiar o transporte real (`STEPUP_TRANSPORT`). Passo-a-passo em `source/agentd/stepup/README.md`. Depois: **cerimônia N=2 real** + decisão **Q5** (ref ao origin) → só então a feature cross-máquina destrava. **R-WP10 segue FECHADO** (verde mono-máquina não discharge). Habilitar fine-grained PATs na org `Ideia-Business` segue ação do owner.
>
...
- Sessão salva em: `/Users/gustavolopespaiva/.claude/sessions/2026-06-23-ideiaos-ed7816d3-ed66-44e6-9d74-50a41ebe.tmp`
