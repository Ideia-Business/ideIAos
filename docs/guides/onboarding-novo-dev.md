...
- **Consumidor** (a maioria) — trabalha **nos projetos** (nfideia, lapidai, ideiapartner).
...
5. **Clona os repos** em `~/dev/` na branch **`work`** e roda `npm install` em cada:
   - `IdeiaOS` · `lapidai` · `nfideia` · `ideiapartner`
   > ⚠️ O `setup-dev-machine.sh` atual é o bootstrap da **máquina primária** e lista no array
   > `REPOS` (topo do script) também o `cfoai-grupori` — **projeto particular, devs não têm acesso**.
   > Um dev novo deve **editar o array `REPOS`** removendo os repos a que não tem acesso, ou clonar
   > manualmente só os seus projetos.
