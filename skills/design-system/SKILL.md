...
- **OKLCH color palettes derived from a single `--brand-hue`** (see `references/oklch-tokens.md`)
...
| **OKLCH Tokens (`--brand-hue`)** | `references/oklch-tokens.md` |
...
4. **Prefer OKLCH for primitives** — derive the whole palette from a single `--brand-hue` (see `references/oklch-tokens.md`). Perceptually uniform, predictable contrast, P3 gamut. Use HSL only where opacity-channel ergonomics matter in legacy stacks.
